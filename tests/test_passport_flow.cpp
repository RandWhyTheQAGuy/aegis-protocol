...
int main() {
    ...
    std::vector<uint8_t> test_key = {
        0x41, 0x65, 0x67, 0x69, 0x73, 0x53, 0x65, 0x63, 0x72, 0x65, 0x74
    }; // "AegisSecret"
    ...
    std::vector<uint32_t> key_ids = {5050};
    ...
    p_no_anchor.log_root_hash.clear(); // Simulate a passport that bypassed the log

    // Re-sign the passport so it's cryptographically valid, but unanchored.
    // Use the same key material the registry used (stored in the passport).
    p_no_anchor.signature = hmac_sha256_hex(
        p_no_anchor.signing_key_material,
        p_no_anchor.content_hash()
    );

    VerifyResult res_no_anchor = registry.verify(p_no_anchor);
    std::cout << "  > Verification Status: " << res_no_anchor.status_str()
              << " (Expected: LOG_MISMATCH)" << std::endl;
    ...
    std::cout << "  > Verification Status: " << res_tampered.status_str()
              << " (Expected: INVALID_SIGNATURE)" << std::endl;
    ...
    const std::string model_to_revoke = p_valid.model_id; // "agent-alpha"
    ...
    std::cout << "  > Verification Status: " << res_revoked.status_str()
              << " (Expected: REVOKED)" << std::endl;
